...
import { MemoizedAdsMultiSelectModal } from "./AdsMultiSelectModal";
...
export default function MultiSelectModal(props: {
  filters: Filter;
  val: any;
  continue: boolean;
}) {
  ...
  if (isFilterDefined) {
    ModalOptions = _filterOptions
      .filter((item) => {
        return !selectedFilter[filterId].some((existItems: any) => {
          return existItems.id === item.id;
        });
      })
      .map((item) => {
  ...
  useEffect(() => {
    if (isFilterDefined) {
      ...
      selectedFilter[filterId]?.map(
        (item: { id: number; levelId: number; label: string }) => {
          if (item.levelId) {
